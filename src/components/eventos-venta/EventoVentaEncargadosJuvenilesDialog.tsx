...
import { EventoVentaEncargadoJuvenilItem } from '@/types/eventos-venta';

interface EventoVentaEncargadosJuvenilesDialogProps {
  visible: boolean;
  loading: boolean;
  searching: boolean;
  submitting: boolean;
  ...
  assigned: EventoVentaEncargadoJuvenilItem[];
  options: EventoVentaEncargadoJuvenilItem[];
  onHide: () => void;
...
export function EventoVentaEncargadosJuvenilesDialog({
  visible,
  loading,
  searching,
  submitting,
  ...
  onAssign,
  onRemove,
}: EventoVentaEncargadosJuvenilesDialogProps) {
  ...
  const filteredAssigned = useMemo(() => {
    ...
    return assigned.filter((item) => {
      const fullName = `${item.apellidos}, ${item.nombre}`.toLowerCase();
      ...
      <div className="grid max-h-[70vh] grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="flex min-h-0 flex-col gap-3">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="eventos-venta-encargados-current-search">
                Filtrar asignados
              </label>
              <IconField iconPosition="right" className="w-full">
                <InputText
                  id="eventos-venta-encargados-current-search"
          ...
          {loading ? <div>Cargando encargados...</div> : null}
          ...
          <ScrollPanel className="h-96 w-full">
            <div className="flex flex-col gap-2 pr-3">
              ...
              {!loading && assigned.length === 0 ? (
                <div>No hay encargados juveniles asignados.</div>
              ) : null}
              {!loading && assigned.length > 0 && filteredAssigned.length === 0 ? (
                <div>No hay encargados asignados que coincidan con el filtro.</div>
              ) : null}
            </div>
          </ScrollPanel>
        </div>
        ...
        <div className="flex min-h-0 flex-col gap-3">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="eventos-venta-encargados-search">
                Buscar protagonista
              </label>
              <IconField iconPosition="right" className="w-full">
                <InputText
                  id="eventos-venta-encargados-search"
                  value={search}
                  onChange={(event) => {
