...
interface Props {
  ...
  loading: boolean;
  submitting: boolean;
  values: TipoEventoFormValues;
  error: string;
  onHide: () => void;
  onSubmit: (values: TipoEventoFormValues) => void;
}
...
export function TipoEventoFormDialog({
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<TipoEventoFormValues>({
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="tipo-evento-nombre">Nombre <span className="text-red-500">*</span></label>
            <Controller name="nombre" control={control} rules={{ required: 'El nombre es obligatorio.' }} render={({ field }) => <InputText id="tipo-evento-nombre" {...field} className="w-full" />} />
            {errors.nombre ? <small className="text-red-500">{errors.nombre.message}</small> : null}
          </div>
          <div className="flex flex-col gap-2">
            <label htmlFor="tipo-evento-descripcion">Descripción</label>
            <Controller name="descripcion" control={control} render={({ field }) => <InputText id="tipo-evento-descripcion" {...field} className="w-full" />} />
          </div>
        </div>
      )}
    </Dialog>
